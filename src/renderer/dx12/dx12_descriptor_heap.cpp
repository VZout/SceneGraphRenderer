...
void Create(DescriptorHeap** heap, Device* device, DescriptorHeapCreateInfo const & create_info) {
	DescriptorHeap* new_heap = new DescriptorHeap();

	new_heap->create_info = create_info;
	new_heap->increment_size = device->native->GetDescriptorHandleIncrementSize(static_cast<D3D12_DESCRIPTOR_HEAP_TYPE>(create_info.type));
	...
	HRESULT hr = device->native->CreateDescriptorHeap(&heap_desc, IID_PPV_ARGS(&new_heap->native));
