...
void Create(ConstantBuffer** buffer, Device* device, unsigned int size) {
	auto* new_buffer = new ConstantBuffer();
	new_buffer->num_buffers = 3; //TODO: Don't hardcode this
	...
	for (unsigned int i = 0; i < 3; ++i) {
		hr = device->native->CreateCommittedResource(
			...
			nullptr,
			IID_PPV_ARGS(&new_buffer->buffer[i]));
		new_buffer->buffer[i]->SetName(L"Constant Buffer Upload Resource Heap");
		if (FAILED(hr)) {
			throw "Failed to create constant buffer resource";
		}

		CD3DX12_RANGE readRange(0, 0);
		hr = new_buffer->buffer[i]->Map(0, &readRange, reinterpret_cast<void**>(&new_buffer->adress[i]));
		if (FAILED(hr)) {
...
void Update(ConstantBuffer* buffer, unsigned int frame_index, unsigned int size, void* data) {
	unsigned int mul_size = (size + 255) & ~255;
	std::memcpy(buffer->adress[frame_index], data, mul_size);
}
