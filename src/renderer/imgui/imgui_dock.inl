...
void LetsDock(bool use_last_as_dest, DockStyle style)
{
	DockContext::Dock* dest = nullptr;
	if (use_last_as_dest) {
		if (!g_dock.m_last_letsdock) {
			throw "their is no last letsdock dock";
		}
		dest = g_dock.m_last_letsdock;
	}
	else {
		dest = g_dock.getRootDock();
	}

	g_dock.m_current->setParent(nullptr);
	g_dock.m_last_letsdock = g_dock.m_current;

	switch (style) {
	case DockStyle::TOP:
		g_dock.doDock(*g_dock.m_current, dest, DockContext::Slot_::Slot_Top);
		break;
	case DockStyle::BOTTOM:
		g_dock.doDock(*g_dock.m_current, dest, DockContext::Slot_::Slot_Bottom);
		break;
	case DockStyle::LEFT:
		g_dock.doDock(*g_dock.m_current, dest, DockContext::Slot_::Slot_Left);
		break;
	case DockStyle::RIGHT:
		g_dock.doDock(*g_dock.m_current, dest, DockContext::Slot_::Slot_Right);
		break;
	case DockStyle::CENTER:
		g_dock.doDock(*g_dock.m_current, dest, DockContext::Slot_::Slot_Tab);
		break;
	}
}
