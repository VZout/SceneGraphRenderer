...
void TextEditor::Render(const char* aTitle, const ImVec2& aSize, bool aBorder)
{
	...
	if (ImGui::IsWindowFocused())
	{
		...
		if (!IsReadOnly() && ctrl && !shift && !alt && ImGui::IsKeyPressed('Y'))
			Redo();
		if (!IsReadOnly() && ImGui::IsKeyPressed(io.KeyMap[ImGuiKey_Enter])) {
			auto pos = GetCursorPosition();
			InsertLine(++pos.mLine);
			SetCursorPosition(pos);
		}
		if (!IsReadOnly() && ImGui::IsKeyPressed(io.KeyMap[ImGuiKey_Tab]))
			InsertText("\t");
	...
	if (!mLines.empty())
	{
		while (lineNo <= lineMax)
		{
			...
			drawList->PushTextureID(ImGui::GetFont()->ContainerAtlas->TexID);

			drawList->AddText(ImGui::GetFont(), ImGui::GetFontSize(), ImVec2(lineStartScreenPos.x /*+ mCharAdvance.x * 1*/, lineStartScreenPos.y), mPalette[(int)PaletteIndex::LineNumber], buf);
