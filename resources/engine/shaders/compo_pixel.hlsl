Texture2D deferred_texture : register(t0);
Texture2D overdose_texture : register(t2);
...
float4 main(VS_OUTPUT input) : SV_TARGET
{
	...
	else if (tonemapping == 3) result = whitePreservingLumaBasedReinhardToneMapping(hdr, gamma);
	else if (tonemapping == 4) result = RomBinDaHouseToneMapping(hdr, gamma);
	else if (tonemapping == 5) result = filmicToneMapping(hdr);
	else if (tonemapping == 6) result = Uncharted2ToneMapping(hdr, exposure, gamma);
	else if (tonemapping == 7) result = GrayscaleToneMapping(hdr);
